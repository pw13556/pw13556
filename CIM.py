...
df['Source'].replace(["Group Internal Audit Department (GIAD) audit findings",
                      ...
                      "Loss events' root cause analysis",'Regulatory audit findings','Others','Compliance Monitoring Review Findings'], 
...
["IT","ITSG","ITSG","ITIM","OP","CO","CO","PGD","CPO","APM","DB","INB","I&O RA","EAS"],inplace=True)
...
df = df.rename(columns={'Country': 'Division', 'SNC': 'PassD','Islamic Business':'Pass Due?','Tag':'y.notificationdate','Tag1':'y.issueindentifiedate',
...
# get year/month form dateobject
...
# pivoting
df1 = pd.pivot_table(df, values='Issue ID', index='y.notificationdate', columns='Source', aggfunc=pd.Series.nunique,) #pd.Series.nunique
df2 = pd.pivot_table(df, values='Issue ID', index='y.notificationdate', columns=['Pass Due?','Classification'], aggfunc=pd.Series.nunique,) #pd.Series.nunique
df0 = pd.pivot_table(df, values='Issue ID', index=['Source','Issue Status','Pass Due?'], columns='Classification', aggfunc=pd.Series.nunique,) #pd.Series.nunique
df3 = df0.xs(('MSII','Open'), level=0)
df4 = pd.pivot_table(df, values='Issue ID', index='y.notificationdate', columns=['Pass Due?','Classification'], aggfunc=pd.Series.nunique,) #pd.Series.nunique
